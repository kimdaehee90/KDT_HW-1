...
crawler = NaverNewsCrawler(input('수집을 원하는 키워드를 입력해주세요 : '))

#### 수집한 데이터를 저장할 엑셀 파일명을 input을 이용해 입력받아 ? 부분에 넣으세요
crawler.get_news(input('수집한 자료를 저장할 Excel 파일명과 확장자를 입력해주세요: '))
...
ws = wb.active
  ...
  ## 엑셀파일 컬럼 구성
ws.append(['Number', 'Name', 'Address'])

  ## append를 통해 메일 수신자 정보 입력
ws.append([1, '유인성', 'isyoo1213@naver.com'])
...
자동화로 보내지는 메일입니다.'''
  ...
  ## email_list.xlsx파일 내 유저에게 메일 발송
for row in data.iter_rows(min_row=2, min_col=2) :
  ## email_list.xlsx 파일 내 user와 addr 데이터 가져오기
    user = row[0].value
    addr = row[1].value
  ## NaverNesCrawler.py에 클래스변수 attachment선언 후, 기사를 저장한 파일 이름을 가져와 parameter로 전달
    send_mail(user, addr, subject, contents, NaverNewsCrawler.attachment)
